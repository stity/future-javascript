...
toc_progress.toggle=function()
{
    ...
    setTimeout(function () {toc_progress.updatemargin();}, 10);
};
...
toc_progress.updatemargin = function () {
    var footer = document.getElementById('toc-progress-footer');
    var main = document.querySelector('body > div');
    var progress = document.querySelector(".progress");
    var width = 0;
    if (footer) {
        width = footer.clientWidth;
        if (getComputedStyle(footer).getPropertyValue('opacity') == "0") {
            width = 0;
        }
    }
    main.style.width="calc(100% - "+width+"px)";
    main.style['margin-left']=width+"px";
    progress.style['margin-left']=width+"px";
}
...
toc_progress.initialize=function(reducescroll,background) {
    ...
    Reveal.addEventListener('slidechanged',function(event){
        toc_progress.updatemargin();
        toc_progress.reduceorscrollifnecessary(this.reduceorscroll)
    });
};
